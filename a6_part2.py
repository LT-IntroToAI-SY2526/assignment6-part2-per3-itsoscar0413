...
def visualize_features(data):
    ...
    axes[0, 0].set_xlabel('SquareFeet (Unit Area)')
    ...
    axes[0, 1].set_xlabel('Bedrooms (Room Count)')
    ...
    axes[1, 0].set_xlabel('Bathrooms (Room Count)')
...
def make_prediction(model, sqft, bedrooms, bathrooms, age):
    ...
    print(f"\n=== New Prediction ===")
    print(f"House specs: {sqft:.0f}k square feet, {bedrooms} number bedroom(s), {bathrooms} bathroom(s), {age} years old")
...
if __name__ == "__main__":
    ...
    make_prediction(model, 2500,5,3,12)
